...
export default function PodcastSlide({
  ...
  const handleMuteToggle = (event: React.MouseEvent<HTMLButtonElement>) => {
    event.stopPropagation(); // Prevents click from triggering other handlers
    const audioElement = audioRef.current;
    if (!audioElement) return;
    ...
    <>
      ...
      >
        ...
        <div className="absolute flex flex-col left-0 top-0 h-full w-full px-10 pt-8 pb-20 items-center">
          ...
          <div className="aspect-square my-4 w-9/12 h-fit">
            {imageUrl ? (
              <img src={imageUrl} className="" alt="Podcast" />
            ) : null}
          </div>
          <audio autoPlay muted={isMuted} src={audioUrl} ref={audioRef} />
          <TextBox text={text} textsize="2xl" position="center" />
        </div>
        <button
          onClick={handleMuteToggle}
          className="absolute bottom-4 right-4 text-2xl z-50"
        >
          <i
            className={`fa-solid ${
